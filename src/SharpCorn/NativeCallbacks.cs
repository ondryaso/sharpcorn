...
public delegate void CodeHookNativeCallback(UIntPtr engine, ulong address, uint size, nuint userData);

// typedef void (*uc_cb_hookintr_t)(uc_engine *uc, uint32_t intno, void *user_data);
[UnmanagedFunctionPointer(CallingConvention.Cdecl)]
public delegate void InterruptHookNativeCallback(UIntPtr engine, uint interruptNumber, nuint userData);

// typedef bool (*uc_cb_hookinsn_invalid_t)(uc_engine *uc, void *user_data);
[UnmanagedFunctionPointer(CallingConvention.Cdecl)]
public delegate bool InvalidInstructionHookNativeCallback(UIntPtr engine, nuint userData);
...
[UnmanagedFunctionPointer(CallingConvention.Cdecl)]
public delegate ulong MMIOReadNativeCallback(UIntPtr engine, ulong offset, uint size, nuint userData);
